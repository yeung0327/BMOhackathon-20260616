...
### 1. 粒度控制 — 只抽核心实体
- 每段文本只抽取：项目名、产品名、技术方案、组织/团队、人物、核心概念（如算法名、架构名）
- 不要抽取：具体的数字指标、日期、版本号、形容词短语、动作描述、实现细节
- 每份文档抽取的实体数量控制在 15-30 个，宁少勿多
- 如果一个概念只是对另一个概念的补充说明，将其作为属性而非独立实体

### 2. 强制连通 — 每个实体必须有关系
- 抽取的每个实体必须至少与另一个实体有直接关系，绝不允许孤立节点
- 如果两个实体没有明确的直接关系，通过文档的核心主题作为桥梁将它们连接
- 同一文档内的实体应形成连通图，不允许出现断开的子图

### 3. 实体归一化 — 统一命名
- 同一概念的不同表述必须统一为一个实体名（如"PP-OCR"和"PP-OCRv6"统一为"PP-OCR"）
- 缩写和全称统一为更常用的那个
- 优先使用简短、明确的实体名

### 4. 中文命名
- 所有关系类型必须使用中文（如：属于、开发了、依赖于、包含、用于、基于、支持、优化了）
- 实体名称使用中文，除非是专有技术术语（如 ONNX、TensorRT）

### 5. 层级结构
- 建立层级：主题 → 子主题 → 关键技术/方案
- 使用"包含"、"属于"、"基于"等关系形成树状结构
- 优先识别从属、因果、依赖关系"""
