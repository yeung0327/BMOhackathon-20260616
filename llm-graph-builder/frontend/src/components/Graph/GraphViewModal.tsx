...
import { filterData, getCheckboxConditions, getSize, graphTypeFromNodes, processGraphData } from '../../utils/Utils';
import { useCredentials } from '../../context/UserCredentials';

import { getGraphSchema, graphQueryAPI, getNeighbors } from '../../services/GraphQuery';
...
const GraphViewModal: React.FunctionComponent<GraphViewModalProps> = ({
  ...
  const [graphHistory, setGraphHistory] = useState<Array<{ nodes: ExtendedNode[]; rels: Relationship[] }>>([]);
  const [exploringNeighbors, setExploringNeighbors] = useState<boolean>(false);
  const [highlightedEntityIds, setHighlightedEntityIds] = useState<string[]>([]);
  const { messages, setMessages, isDeleteChatLoading } = useMessageContext();
  ...
  const handleNodeDoubleClick = useCallback(async (clickedNode: Node) => {
    setExploringNeighbors(true);
    try {
      const result = await getNeighbors(clickedNode.id);
      if (result?.data?.data) {
        const { nodes: nNodes = [], relationships: nRels = [] } = result.data.data;
        if (nNodes.length > 0) {
          // Save current state to history
          setGraphHistory((prev) => [...prev, { nodes: node, rels: relationship }]);
          const { finalNodes, finalRels, schemeVal } = processGraphData(nNodes, nRels);
          setNode(finalNodes);
          setRelationship(finalRels);
          setAllNodes(finalNodes);
          setAllRelationships(finalRels);
    ...
    setHighlightedEntityIds([]); // Clear highlights on explore
  }, [node, relationship]);

  const handleEntityHighlight = useCallback((entityIds: string[]) => {
    setHighlightedEntityIds(entityIds);
    // Apply highlighting to matching nodes
    setNode((prevNodes) =>
      prevNodes.map((n) => {
        const isHighlighted = entityIds.some(
          (eid) => n.caption?.toLowerCase().includes(eid.toLowerCase()) || n.id === eid
        );
        return isHighlighted
          ? { ...n, activated: true, size: (n.size || 30) * 1.5 }
          : { ...n, activated: false, size: getSize({ labels: n.labels }) || undefined };
      })
    );
  }, []);
  ...
  const handleGoBack = useCallback(() => {
    if (graphHistory.length > 0) {
      ...
      setNode(prev.nodes as ExtendedNode[]);
      setRelationship(prev.rels as ExtendedRelationship[]);
      setSelected(undefined);
    }
  }, [graphHistory]);
  ...
  const mouseEventCallbacks = useMemo(
    () => ({
      ...
      onNodeDoubleClick: (clickedNode: Node) => {
    ...
    [selected, handleNodeDoubleClick]
  );
    ...
    <>
      ...
      >
        ...
        <Dialog.Content className='flex flex-col n-gap-token-4 w-full grow overflow-auto border! border-palette-neutral-border-weak!'>
          ...
          <div className='flex w-full h-full max-h-full' style={{ gap: 0 }}>
            {/* Graph area */}
            <div className={`bg-palette-neutral-bg-default relative h-full border! border-palette-neutral-border-weak! ${showChatPanel ? 'w-[65%]' : 'w-full'}`} style={{ transition: 'width 0.3s ease' }}>
            {loading ? (
                  ...
                  <div className='bg-palette-neutral-bg-default relative' style={{ width: '100%', flex: '1' }}>
                    ...
                    <IconButtonArray orientation='vertical' isFloating={true} className='absolute bottom-4 right-4'>
                      {graphHistory.length > 0 && (
                        <IconButtonWithToolTip
                          label='Go Back'
                          text='Go back to previous view'
                          onClick={handleGoBack}
                          placement='left'
                        >
                          <ArrowUturnLeftIconOutline className='n-size-token-7' />
                        </IconButtonWithToolTip>
                      )}
            ...
            {/* Toggle chat button when panel is hidden */}
            {!showChatPanel && (
              <IconButtonArray orientation='vertical' isFloating={true} className='absolute top-4 left-4'>
                <IconButtonWithToolTip
                  label='Show Chat'
            ...
            {/* Chat panel */}
            {showChatPanel && (
              <div className='w-[35%] h-full flex flex-col border-l! border-palette-neutral-border-weak!' style={{ minWidth: '300px' }}>
                <div className='flex items-center justify-between px-3 py-2 border-b! border-palette-neutral-border-weak!'>
                  <span className='font-medium text-sm'>💬 智能问答</span>
                  <IconButtonWithToolTip
                    label='Hide Chat'
                    text='Hide chat panel'
                    onClick={() => setShowChatPanel(false)}
                    placement='left'
                  >
                    <ChevronRightIconOutline className='n-size-token-5' />
                  </IconButtonWithToolTip>
                </div>
                <div className='flex-1 overflow-hidden'>
                  <Chatbot
                    ...
                    isLoading={messages.some((msg) => msg.isTyping || msg.isLoading)}
